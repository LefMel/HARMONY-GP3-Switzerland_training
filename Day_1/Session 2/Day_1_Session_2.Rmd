...
rmarkdown::render('Day_1_Session_2.Rmd', 'beamer_presentation', params=list(presentation=TRUE))
```
...
If this condition is fulfilled, any combination of $S$ and $R$ may allow to estimate $Se$ and $Sp$, e.g. (2T, 2P), (3T,1P), (4T,1P), ...
  ...
  * This condition is describes **Model Identifiability** and is a necessary but not sufficient condition - as we'll see later.
...
model{
  for(p in 1:Populations){
    ...
    # Test1+ Test2+ Pop1
    prob[1, p] <- (prev[p] * (se[1]*se[2]) + ((1-prev[p]) * ((1-sp[1])*(1-sp[2])))
    ## snip ##
  ...
  for(p in 1:Populations){
    ...
    # Test1- Test2+
  	prob[3,p] <- (prev[p] * ((1-se[1])*(se[2]))) + ((1-prev[p]) * ((sp[1])*(1-sp[2])))
  	 # Test1- Test2-
	  prob[4,p] <- (prev[p] * ((1-se[1])*(1-se[2]))) + ((1-prev[p]) * ((sp[1])*(sp[2])))
